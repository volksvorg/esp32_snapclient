...
int wire_chunk_message_deserialize(wire_chunk_message_t *msg, const char *data, uint32_t size){
	...
	tmp = ((uint32_t)buff[3]<<24) | ((uint32_t)buff[2]<<16) | ((uint32_t)buff[1]<<8) | ((uint32_t)buff[0]);
	msg->timestamp.sec=tmp;
	tmp = ((uint32_t)buff[7]<<24) | ((uint32_t)buff[6]<<16) | ((uint32_t)buff[5]<<8) | ((uint32_t)buff[4]);
	msg->timestamp.usec=tmp;
	tmp = ((uint32_t)buff[11]<<24) | ((uint32_t)buff[10]<<16) | ((uint32_t)buff[9]<<8) | ((uint32_t)buff[8]);
	msg->size = tmp;
	int result = 0;
